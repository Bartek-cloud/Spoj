...
int main()
{
    ...
    while(tests--){
        ...
        for(int i = 0 ; i < num_ants_go_right ; i++){
            cin>>ants_go_right[i];
            if(ants_go_right[i]<holes[j]){
                holes_res[j]++;
                if(holes_last_use_time[j]<=0){
                    holes_last_use_time[j]=holes[j]-ants_go_right[i];
                }
            }
            else{
                j++;
        ...
        for(int i = 0 ; i < num_ants_go_left ; i++){
            cin>>ants_go_left[i];
            if(ants_go_left[i]<holes[j+1]){
                holes_res[j]++;
                //jak to zrobic lepej
            }
            else{
                holes_last_use_time[j]=max(ants_go_left[i-1]-holes[j],holes_last_use_time[j]);
                j++;
                holes_res[j]++;
